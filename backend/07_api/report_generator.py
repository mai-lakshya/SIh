"""
NEXUS-XAI Executive Risk Memo PDF Generator
National Risk Platform — Ministry of Rural Development · SIH
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count: int):
        ...
        self.setFillColor(colors.HexColor("#64748B"))
        self.drawString(36, 24, "Survey of India Compliant Cartography · RFCTLARR Act 2013 Statutory Governance · NEXUS-XAI")
...
class ReportGenerator:
    ...
    def generate_pdf_bytes(
        ...
        # =========================================================================
        report_id = f"NEXUS-REP-{time.strftime('%Y%m%d')}-{str(uuid.uuid4())[:8].upper()}"
        gen_time = narrative.get("generated_at", time.strftime("%Y-%m-%d %H:%M UTC", time.gmtime()))

        header_data = [
            [
                Paragraph("NATIONAL RISK PLATFORM · NEXUS-XAI", self.styles["MemoSuperHeader"]),
        ...
        provenance = narrative.get("provenance", "NEXUS Trained Model Engine (Internal)")
